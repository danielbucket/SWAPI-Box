...
describe('Vehicle test', () => {
  const wrapper = mount(<Vehicle isFavorite="favorite"
                                       data={ {name         : 'tie-figher',
                                              model         : "T-5",
                                              vehicle_class : "figher",
                                              passengers    : ["pilot"]} }
  />)

  it('renders without crashing', () => {
    const div = document.createElement('div');
    ReactDOM.render(<Vehicle isFavorite="favorite"
                                   data={ {name          : 'tie-figher',
                                           model         : "T-5",
                                           vehicle_class : "figher",
                                           passengers    : ["pilot"]} }
  ...
  it('should have a class dependent on the prop isFavorite', () => {
    ...
    const newWrapper =mount(<Vehicle isFavorite="not-favorite"
                                           data={ {name          : 'tie-figher',
                                                   model         : "T-5",
                                                   vehicle_class : "figher",
                                                   passengers    : ["pilot"]} }
  ...
  it('should regonize a click for favorites',()=>{
    const spy = jest.fn()
    const wrapper = mount(<Vehicle isFavorite="not-favorite"
                                         data={ {name           : 'tie-figher',
                                                 model          : "T-5",
                                                 vehicle_class  : "figher",
                                                 passengers     : ["pilot"]} }
